...
export function SolverResults({ result, sessionCount, timetableName, onNameChange, onSave, onEdit, saving, saved }: Props) {
    ...
    return (
        <div className={`rounded-xl shadow-sm border p-6 mb-5 ${feasible ? 'bg-green-50 border-green-200' : 'bg-amber-50 border-amber-200'}`}>
            ...
            {/* Stats */}
            <div className="grid grid-cols-2 sm:grid-cols-7 gap-3 text-center mb-4">
                ...
                <ResultStat icon={<Clock className="w-4 h-4" />} label="Prof Same-Half" value={String(result.fitness.professorSameHalfPenalty)} />
