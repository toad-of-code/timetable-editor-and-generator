...
\begin{itemize}
    ...
    \item \textbf{Professor Same-Half Compactness:} The solver encourages compact faculty schedules by penalizing cases where a professor must teach in both the morning and the afternoon on the same day. Let $\mathcal{T}_{p,d}$ be the set of time slots taught by professor $p$ on day $d$. We define the morning half $\mathcal{M} = \{1, 2, 3, 4\}$ and the afternoon half $\mathcal{A} = \{5, 6, 7, 8\}$. The faculty penalty $P(S)$ is:
    \begin{equation}
        P(S) = \sum_{p} \sum_{d} \mathbb{I}\left( (\mathcal{T}_{p,d} \cap \mathcal{M} \neq \emptyset) \land (\mathcal{T}_{p,d} \cap \mathcal{A} \neq \emptyset) \right)
    \end{equation}
\end{itemize}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{Figures/2.png} % Placeholder: Replace \fbox{...} with 
\caption{Fitness Score vs. Generations }
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Figures/3.png} % Placeholder: Replace \fbox{...} with \includegraphics[width=\textwidth]{your_image.png}
\caption{Main Editor UI Grid Surfacing Real-Time Clashes}
\label{fig:main_editor_grid}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Figures/1.png} % Placeholder: Replace \fbox{...} with 
\caption{Final PDF Timetable Output Generated Client-Side}
\label{fig:final_pdf_output}
\end{figure}
